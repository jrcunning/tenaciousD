...
## Analyze Fv/Fm data
We utilize generalized additive mixed models (GAMMs) to model relative Fv/Fm values as a smooth function of time, with dominant symbiont clade as a fixed effect and mother colony and core as random effects. We fit separate models for the cooling treatment and the heating treatment. After fitting the models, we simulate (n=1000) from the posterior distribution of the parameters of the fitted models to generate 95% confidence intervals on fitted values. To test whether Fv/Fm differs between C-dominated and D-dominated corals, we also generate a 95% confidence interval on the difference between these groups from the simulated values, and test whether it includes zero.
